...
const NAV_ITEMS = [
  ...
  { label: "Post Opportunity", icon: "add-circle-outline", route: "PostJob" },
  { label: "Job Posts", icon: "briefcase-outline", route: "Jobs" },
  { label: "Find Candidates", icon: "search-outline", route: "FindCandidates" },
  { label: "Applications", icon: "layers-outline", route: "Applications" },
  { label: "Chat", icon: "chatbubble-outline", route: "Chat" },
  { label: "Profile", icon: "business-outline", route: "Profile" },
] as const;
...
const EmployerSidebar = ({ visible, onClose }: SidebarProps) => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Navigation Items */}
          {NAV_ITEMS.map((item) => {
            const isActive = pathname === `/employer/${item.route}`;
            return (
              <TouchableOpacity
                key={item.label}
                onPress={() => {
                  router.push(`../employer/${item.route}`);
                  onClose();
                }}
                activeOpacity={0.7}
